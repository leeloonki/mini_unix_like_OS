...
#define SELECTOR_U_CODE ((5<<3) + ( TI_GDT <<2 ) + RPL3 ) //内核显存段 GDT 第5项
#define SELECTOR_U_DATA ((6<<3) + ( TI_GDT <<2 ) + RPL3 ) //内核数据段 GDT 第6项
...
/************************************ eflags属性 *******************************/
#define EFLAGS_2 (1<<1)                 //第2位固定为1
#define EFLAGS_IF_1 (1<<9)
#define EFLAGS_IF_0 (0<<9)
#define EFLAGS_IOPL_3 (3<<12)           //IOPL3测试用户程序在非系统调用下进程IO
...
// 除法向上取整
#define DIV_ROUND_UP(X,STEP) (((X-1)/STEP) + 1)
