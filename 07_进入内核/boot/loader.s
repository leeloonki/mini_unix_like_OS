...
; -------------------------------------------------
    ...
    ; eax 起始盘块   KERNEL_START_SECTOR定义在/include/boot.inc
    ; × bx loader从硬盘读到内存位置[bx]   bx 16位寄存器 最多读到0xffff
    ;   bx超ffff后变为0 ds:bx = 0 若读取字过多会覆盖0x900当前栈(<0xffff)
    ; √ ebx loader从硬盘读到内存位置[ebx]   ebx 32位寄存器偏移4G不会出错
    ; 
    ; cx 读入扇区数 = count = 200,实际不会读这么多
    mov eax,KERNEL_START_SECTOR ;
    mov ebx,KERNEL_BIN_BASE_ADDR;
    mov ecx,200
    call rd_disk_m_32           ;分页前读取kernel.bin
...
; ---------------------------------------------------------------
; 将kernel.bin中的程序头's 拷贝到编译的虚拟地址处
; ---------------------------------------------------------------
...
kernel_init:
    ...
    mov edx,[KERNEL_BIN_BASE_ADDR+42]  ;e_phentsize 相对elf32_ehdr结构体地址16+2+2+4+4*3+4+2=42
    ; e_phoff 程序头表起始地址
