...
void pfree(uint32_t pg_phy_addr){
    ...
    bitmap_set(&mem_pool,bit_idx,0);    //将该页对应位图置0
}
...
static void page_table_pte_remove(uint32_t vaddr){
    ...
    *pte &= ~PG_P_1;                    //将p位置0
    asm("invlpg %0"::"m"(vaddr):"memory");                         //刷新TLB vaddr对应的缓存
}
...
void mfree_page(enum pool_flags pf,void* _vaddr,uint32_t pg_cnt){
    ...
    pg_phy_addr = addr_v2p(vaddr);          //获取虚拟地址对应物理地址
    ...
    if( pg_phy_addr >= user_pool.phy_addr_start){//位于用户物理内存池
        ...
        while(page_cnt<pg_cnt){                 //回收vaddr开始的pg_cnt个用户物理页
            ...
            ASSERT((pg_phy_vaddr % PG_SIZE)==0 && pg_phy_addr >=user_pool.phy_addr_start);
        ...
        while(page_cnt<pg_cnt){                 //回收vaddr开始的pg_cnt个用户物理页
            vaddr+=PG_SIZE;
            pg_phy_addr = addr_v2p(vaddr);
            ASSERT((pg_phy_vaddr % PG_SIZE)==0 && pg_phy_addr >=kernel_pool.phy_addr_start && \
