...
struct task_struct*idle_thread; //ide线程PCB
...
void init_thread(struct task_struct* pthread,char*name,int prio){
    ...
    pthread->stack_magic = 0x20000000;                    //任意
}
...
static void idle(void* arg){
    while(1){
        thread_block(TASK_BLOCKED);         //idle初始化为ready，会运行一次，此后被阻塞，当其被外部中断唤醒后
        asm volatile("sti;hlt":::"memory"); //执行hlt，使系统挂起
    }
}
...
void schedule(){
    ...
    // ASSERT(!list_empty(&thread_ready_list)); //断言有可供调度运行的线程，若无可调度线程则触发ASSERT，
    // 我们顺便在这里添加idle线程，在没有线程可调度时，就调用ide线程
...
void thread_yield(){
    struct task_struct* cur = running_thread();
    enum intr_status old_status = intr_disable();
    ASSERT(!elem_find(&thread_ready_list,&cur->general_tag));//运行的当前线程状态为task_running
    list_append(&thread_ready_list,&cur->general_tag);      //添加到就绪队列
    cur->status = TASK_READY;
    schedule();
    intr_set_status(old_status);
}
